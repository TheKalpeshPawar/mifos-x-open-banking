...
/**
 * Opens the stream of direct-debit mandates for one account.
 *
 ...
 *
 * The stream emits `Content` even when the account has no mandates; callers apply `emptyIfContent`
 * to turn that into an empty screen state.
 */
interface DirectDebitsRepository {

    fun directDebitsStream(
        accountId: String,
