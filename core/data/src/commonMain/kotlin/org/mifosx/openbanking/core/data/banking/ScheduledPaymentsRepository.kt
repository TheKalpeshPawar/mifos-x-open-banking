...
/**
 * Opens the stream of future-dated scheduled payments for one account.
 *
 ...
 *
 * The stream emits `Content` even when the account has no scheduled payments; callers apply
 * `emptyIfContent` to turn that into an empty screen state.
 */
interface ScheduledPaymentsRepository {

    fun scheduledPaymentsStream(
        accountId: String,
