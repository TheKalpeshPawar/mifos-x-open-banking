...
/**
 * Opens the stream of standing orders for one account.
 *
 ...
 *
 * The stream emits `Content` even when the account has no standing orders; callers apply
 * `emptyIfContent` to turn that into an empty screen state.
 */
interface StandingOrdersRepository {

    fun standingOrdersStream(
        accountId: String,
