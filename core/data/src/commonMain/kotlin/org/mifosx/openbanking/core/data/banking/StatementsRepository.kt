...
/**
 * Opens the stream of statement periods for one account.
 *
 ...
 *
 * The stream emits `Content` even when the account has no statements; callers apply `emptyIfContent`
 * to turn that into an empty screen state.
 */
interface StatementsRepository {

    fun statementsStream(
        accountId: String,
