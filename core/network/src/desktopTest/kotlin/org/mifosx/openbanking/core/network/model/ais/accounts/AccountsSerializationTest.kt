 ...
 */
package org.mifosx.openbanking.core.network.model.ais.accounts
...
class AccountsSerializationTest {
    ...
    private fun sampleResponse() = AccountsResponse(
    ...
    @Test
    fun `full accounts response round-trips through JSON`() {
    ...
    fun `account fields survive the round-trip`() {
        ...
        assertEquals("CurrentAccount", account?.accountSubType)
    ...
    fun `data class helpers are exercised`() {
        ...
        val defaults = AccountsResponse()
        assertEquals(AccountsResponse(), defaults)
    }
}
