...
class StatementDetailsSerializationTest {
    ...
    private fun sampleResponse() = StatementDetailsResponse(
    ...
    @Test
    fun `full statement details response round-trips through JSON`() {
        val original = sampleResponse()
        val encoded = json.encodeToString(StatementDetailsResponse.serializer(), original)
        val decoded = json.decodeFromString(StatementDetailsResponse.serializer(), encoded)
        assertEquals(original, decoded)
    }

    @Test
    fun `statement details fields survive the round-trip`() {
        val original = sampleResponse()
        val encoded = json.encodeToString(StatementDetailsResponse.serializer(), original)
        val decoded = json.decodeFromString(StatementDetailsResponse.serializer(), encoded)
        ...
        assertEquals("Credit interest", statement?.statementInterest?.first()?.description)
        assertEquals("AnnualBorrowingRate", statement?.statementRate?.first()?.type)
        assertEquals("/self", decoded.links?.self)
        assertEquals(2, decoded.meta?.totalPages)
    }
    ...
    fun `data class helpers are exercised`() {
        ...
        val interest = statement.statementInterest?.first()
        assertEquals(interest, interest?.copy())
        assertTrue(interest.toString().contains("Credit interest"))
        val defaults = StatementDetailsResponse()
        assertEquals(StatementDetailsResponse(), defaults)
    }
}
