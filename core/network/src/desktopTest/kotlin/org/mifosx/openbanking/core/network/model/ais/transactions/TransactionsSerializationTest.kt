 ...
 */
package org.mifosx.openbanking.core.network.model.ais.transactions
...
class TransactionsSerializationTest {
    ...
    private fun sampleTransaction() = Transaction(
        ...
        chargeAmount = ChargeAmount(amount = "0.50", currency = "GBP"),
    ...
    private fun sampleResponse() = TransactionsResponse(
        data = Data(transaction = listOf(sampleTransaction())),
        links = Links(self = "/self", next = "/next", last = "/last"),
        meta = Meta(totalPages = 3),
    )

    @Test
    fun `full transactions response round-trips through JSON`() {
        val original = sampleResponse()
        val encoded = json.encodeToString(TransactionsResponse.serializer(), original)
        val decoded = json.decodeFromString(TransactionsResponse.serializer(), encoded)
    ...
    fun `data class helpers are exercised`() {
        ...
        val defaults = TransactionsResponse()
        assertEquals(TransactionsResponse(), defaults)
    }
}
