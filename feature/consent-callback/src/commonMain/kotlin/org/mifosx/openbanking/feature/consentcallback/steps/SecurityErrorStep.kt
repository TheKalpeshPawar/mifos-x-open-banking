...
@Composable
internal fun SecurityErrorStep(onAction: (ConsentCallbackAction) -> Unit, modifier: Modifier = Modifier) {
    Box(
        modifier = modifier.fillMaxSize().testTag("callback_security_error"),
        ...
        Column(
            ...
            Icon(Icons.Outlined.Security, null, Modifier.size(64.dp), tint = KptTheme.colorScheme.error)
            Spacer(Modifier.height(24.dp))
            Text(
                stringResource(Res.string.feature_callback_security_error_title),
                style = KptTheme.typography.headlineSmall,
                color = KptTheme.colorScheme.onSurface,
            )
            Spacer(Modifier.height(KptTheme.spacing.sm))
            Text(
                stringResource(Res.string.feature_callback_security_error_body),
