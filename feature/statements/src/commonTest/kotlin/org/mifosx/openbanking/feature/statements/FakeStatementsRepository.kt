...
/**
 * Hand-written [StatementsRepository] whose stream the test drives directly.
 *
...
class FakeStatementsRepository(
    initial: ScreenState<List<StatementPeriod>> = ScreenState.Loading,
) : StatementsRepository {

    private val states = MutableStateFlow(initial)
    private val refreshes = MutableSharedFlow<Unit>(replay = REFRESH_REPLAY)

    /** The account id the view model subscribed with; null until [statementsStream] is called. */
    ...
    fun emit(state: ScreenState<List<StatementPeriod>>) {
        states.value = state
    }
