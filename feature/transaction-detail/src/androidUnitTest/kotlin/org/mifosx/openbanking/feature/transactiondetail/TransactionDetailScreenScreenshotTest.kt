 ...
 * `src/androidUnitTest/screenshots/`; the record task writes them and the verify task fails the build
 * on any pixel drift.
 *
...
class TransactionDetailScreenScreenshotTest {
    ...
    private fun capture(state: String, screenState: TransactionDetailState) {
        ...
        composeRule.onRoot()
            .captureRoboImage("src/androidUnitTest/screenshots/transaction_detail_$state.png")
    }
