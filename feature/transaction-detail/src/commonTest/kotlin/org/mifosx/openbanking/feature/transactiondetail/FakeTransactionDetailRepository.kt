...
/**
 * Hand-written [TransactionDetailRepository] whose stream the test drives directly.
 *
 ...
 */
class FakeTransactionDetailRepository(
    initial: ScreenState<List<TransactionDetail>> = ScreenState.Loading,
) : TransactionDetailRepository {

    private val states = MutableStateFlow(initial)
    private val refreshes = MutableSharedFlow<Unit>(replay = REFRESH_REPLAY)

    /** The account id the view model subscribed with; null until [transactionDetailStream] is called. */
    ...
    fun emit(state: ScreenState<List<TransactionDetail>>) {
        states.value = state
    }
